...
function buildUrl(serletpath, encodedDiagram, index) {
  ...
  // Usar la base configurada (por defecto https://www.plantuml.com/plantuml)
  const base = typeof getPlantumlBase === 'function' ? getPlantumlBase() : 'https://www.plantuml.com/plantuml';
  console.log('🔧 buildUrl - base:', base, 'pathname:', pathname);
  // Si hay base, concatenar con / solo si pathname no empieza con /
  if (base) {
    const fullUrl = pathname.startsWith('/') ? `${base}${pathname}` : `${base}/${pathname}`;
