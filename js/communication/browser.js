...
const { sendMessage, suppressNextMessage, initAppCommunication } = (function() {
  ...
  function sendMessage(data) {
    ...
    console.log("📤 Sending message via BroadcastChannel:", data);
    // Send via BroadcastChannel for cross-context communication
    (new BroadcastChannel(BROADCAST_CHANNEL)).postMessage(data);
    // Also dispatch as custom event for same-page communication
    window.dispatchEvent(new CustomEvent('plantuml-message', { detail: { data: data } }));
  }

  function initAppCommunication() {
    console.log("🔧 Initializing BroadcastChannel communication...");
    ...
    console.log("✅ BroadcastChannel listener registered on channel:", BROADCAST_CHANNEL);
    
    // Also listen for same-page custom events
    window.addEventListener('plantuml-message', (event) => {
      console.log("📨 Received same-page message:", event.detail.data);
